  ...
  <div class="manage-container">
      ...
      <div class="details-content">
        <div class="top-actions">
          ...
          <div class="io-group">
            ...
            <button class="action-btn export-btn" @click.prevent="exportData">📤 导出</button>
            <button class="action-btn clear-all-btn" @click.prevent="showClearConfirmModal = true">🗑️ 清空</button>
          </div>
        </div>
    ...
    <div class="modal-overlay" v-if="showClearConfirmModal" @click.self="showClearConfirmModal = false">
      <div class="nice-modal">
        <h3>🗑️ 确认清空</h3>
        <p>确定要清空<b>所有食谱数据</b>吗？<br>清空后可以方便地导入新数据。此操作不可撤销。</p>
    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal = false">
      <div class="modal-content">
        ...
        <div class="form-group"><label>名称：</label><input type="text" v-model="formData.name" placeholder="例如：金拱门 / 聚餐选项" class="form-input"></div>
        <div class="form-group"><label>初始基础权重 ({{ formData.weight }}分)：</label><input type="range" min="1" max="100" v-model.number="formData.weight" class="slider-input"></div>
        <div class="form-group type-switch"><label class="toggle-label"><input type="checkbox" v-model="formData.isGroup"><b>作为“多级层级群组”？</b></label></div>
        ...
        <div class="form-group" v-else>
          <label>标签：</label>
          <div class="quick-tags" v-if="allAvailableTags.length > 0"><span v-for="tag in allAvailableTags" :key="tag" class="quick-tag" :class="{ selected: formData.tags.includes(tag) }" @click="toggleTag(tag)">{{ tag }}</span></div>
          <div class="add-tag-wrapper"><input type="text" v-model="newTagInput" placeholder="新标签..." @keyup.enter="addNewTag" class="form-input small"><button @click="addNewTag" class="action-btn tag-add-btn">添加</button></div>
        </div>
...
const deleteFood = (id) => { if (confirm("确定要删除这项内容吗？")) saveFoods(allFoods.value.filter(f => f.id !== id)); };
const handleImportedData = (data) => { if (data.foods) saveFoods(data.foods); if (data.rules) saveRules(data.rules); alert("🎉 数据导入成功！"); };
</script>
...
.rules-help p { margin: 6px 0; } .rules-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 15px;}
.rules-grid label { font-size: 12px; font-weight: bold; color: #555; display: flex; flex-direction: column; gap: 4px;}
.rules-grid .full-width { grid-column: span 2; } .save-rules-btn { background: #FF9800; color: white; width: 100%; margin-top: 5px; }
...
.export-btn { background: #2196F3; color: white; width: 100%; }
.clear-all-btn { background: #F44336; color: white; width: 100%; }
...
.food-weight { margin: 0 0 8px 0; font-size: 13px; color: #666; } .food-tags { display: flex; flex-wrap: wrap; gap: 5px; }
...
.small-btn { padding: 8px; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; } .edit { background: #e3f2fd; } .delete { background: #ffebee; }
...
.form-group { margin-bottom: 18px; } .form-group label { display: block; font-weight: bold; margin-bottom: 8px; font-size: 14px; }
.form-input { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; margin-top: 5px; box-sizing: border-box;}
.slider-input { width: 100%; margin: 10px 0; }
.type-switch { background: #fff3e0; padding: 10px; border-radius: 8px; } .toggle-label { display: flex; align-items: center; gap: 8px; cursor: pointer; }
